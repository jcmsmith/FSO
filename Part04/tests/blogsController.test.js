...
describe('Adding a new blog', () => {
  ...
  test('without the likes property will default to likes:0', async () => {
    ...
    const posted = blogsAfterPost.filter(blog => blog.title === 'React patterns')
...
describe('Updating a blog', () => {
  test('succeeds when updating likes only', async () => {
    ...
    const updatedBlog = blogsAfterUpdate.filter(blog => blog.title === oldBlog.title)[0]
  ...
  test('succeeds when updating title only', async () => {
    ...
    const blogsAfterUpdate = await helper.blogsInDb()
    const updatedBlog = blogsAfterUpdate.filter(blog => blog.author === oldBlog.author)[0]
