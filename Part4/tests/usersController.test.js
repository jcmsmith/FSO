...
describe('Creating a new user', () => {
  ...
  test('does not return password or passwordHash upon success', async () => {
    ...
    const usersAtEnd = await helper.usersInDb()
    const returnedUser = usersAtEnd.filter(user => user.username === newUser.username)
  ...
  test('fails with proper status code and message if username is too short', async () => {
    ...
    expect(result.body.error).toContain('Username must be 3 or more characters in length')
  ...
  test('fails with proper status code and message if password is too short', async () => {
    ...
    expect(result.body.error).toContain('Password must be 5 or more characters in length')
